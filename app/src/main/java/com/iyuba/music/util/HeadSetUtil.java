...
public class HeadSetUtil {
    ...
    public void open(OnHeadSetListener headSetListener) {
        ...
        if (ConfigManager.getInstance().isMediaButton()) {
            ComponentName name = new ComponentName(Utils.getMusicApplication().getPackageName(), MediaButtonReceiver.class.getName());
        ...
        public void onAudioFocusChange(int focusChange) {
            switch (focusChange) {
                case AudioManager.AUDIOFOCUS_LOSS:
                    if (Utils.getMusicApplication().getPlayerService().getPlayer().isPlaying()) {
                        // 因为会长时间失去，所以直接暂停
...
//                        mPausedByTransientLossOfFocus = true;
                ...
                case AudioManager.AUDIOFOCUS_GAIN:
                    // 重新获得焦点，且符合播放条件，开始播放
                    if (!Utils.getMusicApplication().getPlayerService().getPlayer().isPlaying() && changeByAudioListener) {
//                        mPausedByTransientLossOfFocus = false;
