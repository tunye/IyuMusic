...
public class SwipeBackLayout extends ViewGroup {
    ...
    private boolean overCriticalPoint = false;
    ...
    public SwipeBackLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        overCriticalPoint = false;
        viewDragHelper = ViewDragHelper.create(this, 0.3f, new ViewDragHelperCallBack());
    }
    ...
    private void findScrollView(ViewGroup viewGroup) {
        ...
        if (viewGroup.getChildCount() > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (child instanceof AbsListView || child instanceof ScrollView || child instanceof ViewPager || child instanceof WebView || child instanceof RecyclerView) {
                    scrollChild = child;
                    return;
                }
            }
    ...
    private class ViewDragHelperCallBack extends ViewDragHelper.Callback {
        ...
        public int clampViewPositionVertical(View child, int top, int dy) {
            ...
            int result = 0;
            if (Math.abs(dy) > 130 || scrollChild == null) {
                overCriticalPoint = true;
            }
            if (dragEdge == DragEdge.TOP && !canChildScrollUp() && top > 0 && overCriticalPoint) {
                final int topBound = getPaddingTop();
                final int bottomBound = verticalDragRange;
                result = Math.min(Math.max(top, topBound), bottomBound);
            } else if (dragEdge == DragEdge.BOTTOM && !canChildScrollDown() && top < 0 && overCriticalPoint) {
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            ...
            int result = 0;
            if (Math.abs(dx) > 130 || scrollChild == null) {
                overCriticalPoint = true;
            }
            if (dragEdge == DragEdge.LEFT && !canChildScrollRight() && left > 0 && overCriticalPoint) {
                final int leftBound = getPaddingLeft();
                final int rightBound = horizontalDragRange;
                result = Math.min(Math.max(left, leftBound), rightBound);
            } else if (dragEdge == DragEdge.RIGHT && !canChildScrollLeft() && left < 0 && overCriticalPoint) {
...
//                    执行finish activity，为保证Destroy执行，在activity设置监听器
//                }
//            }
